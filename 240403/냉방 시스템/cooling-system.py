...
# - 1칸, 3칸, 5칸,,,
# - 직진 바람 -> 그쪽 방향 벽없으면 직진
# - 위쪽 45도 -> 위 + 오, 그래서 둘다 벽이없어야함
# - 시원함은 단순 숫자 합
# - 그렇기에 에어컨이 있는 자리에서도 시원함

# 2. 시원한 공기 섞임
# - 모든 칸에 대해 동시에 일어남
# - 단 벽을 사이에 두고 있는 칸 끼리는 일어나지 않음
# - [시원함의 차이 / 4]

# 3. 외벽에 있는 칸은 시원함 1감소
# - 0이였다면 감소 X
# - 이런 1->2->3 거치면 1분 흐름
# - 전체 시원함이 k이상이 될 때까지 반복
...
# print(check_up_block,check_left_block)
...
def get_dir_ice(dir,cur_index,value,memo): # cur_index에서 dir 방향으로 옮겨가, check_ice에 value을 더하자
    global check_ice
    ...
    if flag:
        ...
        # 그다음 장소로 보내기
        if dir in (0,4,7):
            get_dir_ice(0, next_index, value, memo)
            get_dir_ice(4, next_index,value, memo)
            get_dir_ice(7, next_index, value, memo)
        elif dir in (1,8,9):
            get_dir_ice(1, next_index, value, memo)
            get_dir_ice(8, next_index, value, memo)
            get_dir_ice(9, next_index, value, memo)
        elif dir in (2,5,6):
            get_dir_ice(2, next_index, value, memo)
            get_dir_ice(5, next_index, value, memo)
            get_dir_ice(6, next_index, value, memo)
        else:
            get_dir_ice(3, next_index, value, memo)
            get_dir_ice(10, next_index, value, memo)
            get_dir_ice(11, next_index, value, memo)
def get_ice():
    ...
    for dir in range(4): # 0:왼,1:위,2:오,3:아래
        ...
        for index in current_aircon[dir]:
            ...
            get_dir_ice(dir,next_index,4,memo) # dir방향으로 한칸 움직인 다음에 value을 넣어줘야함
            if dir == 0:
                get_dir_ice(4, next_index, 4,memo)  # dir방향으로 한칸 움직인 다음에 value을 넣어줘야함
                get_dir_ice(7, next_index, 4,memo)  # dir방향으로 한칸 움직인 다음에 value을 넣어줘야함
            elif dir == 1:
                get_dir_ice(8, next_index, 4,memo)  # dir방향으로 한칸 움직인 다음에 value을 넣어줘야함
                get_dir_ice(9, next_index, 4,memo)  # dir방향으로 한칸 움직인 다음에 value을 넣어줘야함
            elif dir ==2:
                get_dir_ice(5, next_index, 4,memo)  # dir방향으로 한칸 움직인 다음에 value을 넣어줘야함
                get_dir_ice(6, next_index, 4,memo)  # dir방향으로 한칸 움직인 다음에 value을 넣어줘야함
            else:
                get_dir_ice(10, next_index, 4,memo)  # dir방향으로 한칸 움직인 다음에 value을 넣어줘야함
                get_dir_ice(11, next_index, 4,memo)  # dir방향으로 한칸 움직인 다음에 value을 넣어줘야함
def decrease_ice():
    global check_ice
    # 1행
    for i in range(n):
